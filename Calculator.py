...
bot = telebot.TeleBot('5501164209:AAETNZjSKbzTGVS5hk6ufFyHsd9MvFKmpPA')
with open('logfile.log', 'a') as f_log:
    print(time(),'Bot activeted', file= f_log)

def do_log(msg: telebot.types.Message):
    with open('logfile.log', 'a') as f_log:
        print(time(),f'User ({msg.from_user.id}) send message: {msg.text}', file= f_log)
value = ''
old_value = ''
